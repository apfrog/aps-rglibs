...
function(startpoint:vec2, endpoint:vec2, acceleration:number, fallspeed:number, cpu:CPU, grounded:boolean)
	if not grounded then 
		acceleration += cpu.DeltaTime
	else
		acceleration = 0
	end
	startpoint += vec2(0, (fallspeed * 9.8 * acceleration))
	endpoint += vec2(0, (fallspeed * 9.8 * acceleration))
	return startpoint, endpoint, acceleration -- Assign in the cpu by doing: value, value1, value2 = physap.gravity(values here)
end
